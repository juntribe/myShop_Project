...
public class MemberFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("/MemberJoin.mm")) {
			// 회원가입 페이지 -> view 페이지
		...
		else if (command.equals("/MemberJoinAction.mm")) {
		...
		else if(command.equals("/LoginCheck.mm")){
			System.out.println("C : /LoginCheck.mm 호출");
			// DB 사용 => MemberLoginAction() 객체를 생성해야함.
			action = new LoginCheck();
			System.out.println("C : 아이디체크 메소드 호출");
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
		...
		else if(command.equals("/admin.mm")){
		...
		else if(command.equals("/admin/admin.mm")){
			HttpSession session =request.getSession();
			String id = (String) session.getAttribute("id");
			if (id == null || !id.equals("admin")){
				response.setContentType("text/html; charset=UTF-8");
		...
		else if(command.equals("/MemberUpdateProAction.mm")){
		...
		else if(command.equals("/admin/MemberList.mm")){
