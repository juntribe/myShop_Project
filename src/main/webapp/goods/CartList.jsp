...
<%@page import="com.Project.goods.vo.Cart"%>
<%@page import="com.Project.goods.vo.Goods"%>
<%@page import="com.Project.goods.dao.GoodsDAO"%>
...
<script>
	...
	function checkQty(name,qty){
		if(qty != 1){
			location.href="CartQtyDown.gd?name="+ encodeURIComponent(name);
		}
		function cartListTotal(cartList){
			var cartListTotalList=0;
			for (var i=0; i<=cartList.length; i++){

				cartListTotalList +=i;
			}
			alert(cartListTotalList);
		}
	}
</script>
</head>
...
<form method="post">
      <table>
    	<tr id="select">
    		...
    		<input type="submit" value="검색" formaction="CartSearch.gd"/>
    		</td>
    	</tr>
        <tr class = "tr_top">
        ...
        <tr>
        	<td>
             ...
             <td>
             <img src = "/image/${cart.image }" id ="cartImage"/>
            </td>
             <td>
             ${cart.name }
            </td>
             ...
             <td>
             <a href="CartQtyUp.gd?name=${cart.encodingName }">
             <img src="common/image/up.png" id = "upImage" border=0/>
             </a><br>
            <div style="margin-left: 4px">${cart.qty }<br></div>

             <a href="javascript:checkQty('${cart.name}','${cart.qty}')">
             <img src="common/image/down.png" id = "downImage" border=0/>
             </a>
            </td>
         </tr>
        </c:forEach>
      ...
      <tr>
         <td colspan="5" style="text-align:center;">
         	<input type="submit" value="삭제" formaction="CartRemove.gd"/>
         </td>
      </tr>
      </table>
</form>
